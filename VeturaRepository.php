<?php
    ...
    class VeturaRepository{
        ...
        public function insertVetura($vetura){
            ...
            $foto = $vetura->getFoto();
            $modifikim = $vetura->getModifikim();

            $sql = "INSERT INTO vetura(Emri, VitiProdhimit, Km, Motori, HP, Cmimi, Foto, Modifikim) VALUES (?,?,?,?,?,?,?,?)";

            $statement = $conn->prepare($sql);
            $statement->execute([$emri, $vitiProdhimit, $km, $motori,$hp, $cmimi, $foto, $modifikim]);
        ...
        public function editVetura($id, $emri, $vitiProdhimit, $km, $motori,$hp, $cmimi, $foto, $modifikim){
            $conn = $this->connection;
            $sql = "UPDATE vetura SET Emri=?,VitiProdhimit=?, Km=?, Motori=?, HP=?, Cmimi=?, Foto=?, Modifikim=? WHERE ID=?";

            $statement = $conn->prepare($sql);
            $statement->execute([$emri, $vitiProdhimit, $km, $motori,$hp, $cmimi, $foto, $modifikim, $id]);
